...
function create(){
    ...
    if (!lowMem){
        ...
        houseShit2 = FlxTween.tween(house, {y: house.y-50}, 2.5, {ease: FlxEase.sineInOut, type: FlxTween.PINGPONG});
...
function stepHit(){
    ...
    if (curSong == 'forgotten-world'){
        switch(curStep){
            ...
            case 2389:
                for (i in [timeBar,timeBarBG, timeTxt, diffTxt, songTxt]){
                    i.alpha = 0.001;
                }
        ...
        if (curStep == 805 || curStep == 870){
            FlxTween.tween(camGame, {zoom: .65}, 0.075, {
                ease: FlxEase.quadInOut,
                onComplete: 
                function (twn:FlxTween)
                    {
